...
const SOCIAL = {
facebook:  "https://www.facebook.com/alnilegourmet/",import { useState, useMemo, useCallback, useRef, useEffect } from "react";
...
const INIT_ADMIN = BASE_ITEMS.reduce((acc, it) => {
  acc[it.id] = { available: true, price: it.price, priceStr: it.priceStr, img: it.img, name: it.name };
  return acc;
}, {});
  ...
  return (
    <div className="shov" onClick={onClose} role="dialog" aria-modal="true">
      <div className="sh" onClick={e => e.stopPropagation()}>
        ...
        <button className="btn-chk" style={{ marginTop: 14 }} onClick={() => {
      ...
      <div className="edit-modal" onClick={e => e.stopPropagation()}>
        ...
        {imgUrl && (
          <div style={{ marginBottom: 14 }}>
            ...
            <SafeImg src={imgUrl} alt="preview" style={{ height: 110, width: "100%", borderRadius: 10, objectFit: "cover", border: "1px solid var(--bdl)" }} />
          </div>
        )}
...
export default function App() {
  ...
  const go = useCallback((p) => {
    setPrevPage(prev => prev);
    setPage(prev => { setPrevPage(prev); return p; });
  ...
  const saveItemEdit = useCallback((id, updates) => {
    const newPrice = parsePrice(updates.priceStr);
    setAdminItems(prev => ({
      ...prev,
      [id]: { ...prev[id], priceStr: updates.priceStr, price: newPrice > 0 ? newPrice : prev[id].price, img: updates.img || prev[id].img }
    }));
    setEditingItem(null);
...
// ─── IMAGES ──────────────────────────────────────────────────────────
const I = Array.from({length: 85}, (_, i) => `https://picsum.photos/seed/alnile${i}/400/300`);
...
const FALLBACK_IMG = "https://placehold.co/400x300/1E1E1E/C9A227?text=%F0%9F%8D%96";
...
// FIX: حماية آلية لاختفاء الصور بسبب الذاكرة المؤقتة (LocalStorage)
const ALL_ITEMS = useMemo(() =>
BASE_ITEMS.map(it => {
const ov = adminItems[it.id];
// إذا كان هناك تعديل محفوظ، نتأكد أن الصورة ليست فارغة وأنها رابط صحيح
const hasValidSavedImg = ov && ov.img && typeof ov.img === 'string' && ov.img.trim() !== "";
const finalImg = hasValidSavedImg ? ov.img : it.img;

return ov ? {
...it,
price: ov.price ?? it.price,
priceStr: ov.priceStr ?? it.priceStr,
img: finalImg,
available: ov.available ?? true
} : { ...it, available: true };
}),
[adminItems]);
